...
/**
 * Garante que o Flyway execute as migrações antes do Hibernate validar o schema.
 * Cria e executa o Flyway manualmente (o bean Flyway do Spring Boot 4 pode não existir).
 * - flywayRunner: executa flyway.migrate() ao iniciar, antes do JPA.
 * - BeanFactoryPostProcessor: faz entityManagerFactory depender de flywayRunner.
 */
@Configuration
public class FlywayBeforeJpaConfig implements BeanFactoryPostProcessor {
    ...
    public FlywayRunner flywayRunner(
            ...
            @Value("${spring.flyway.schemas:agenda}") String schemas,
    ...
    public void postProcessBeanFactory(ConfigurableListableBeanFactory beanFactory) throws BeansException {
        if (!beanFactory.containsBeanDefinition(FLYWAY_RUNNER)) {
            return;
        }
        ...
        if (existing != null && Arrays.asList(existing).contains(FLYWAY_RUNNER)) {
            return;
        }
        String[] updated = existing == null
                ? new String[]{FLYWAY_RUNNER}
    ...
    /**
     * Executa flyway.migrate() na inicialização, antes do JPA.
     */
    public static final class FlywayRunner implements InitializingBean {
        ...
        FlywayRunner(DataSource dataSource, String locations, String schemas,
            ...
            this.schemas = schemas != null ? schemas : "agenda";
        ...
        public void afterPropertiesSet() {
            log.info("Executando migrações Flyway antes do JPA...");
